...
import com.bhhan.testutils.MultiplicationApplication;
import com.bhhan.testutils.beans.AttemptResponse;
import com.bhhan.testutils.beans.Stats;
import cucumber.api.java.Before;
import cucumber.api.java.ko.그러면;
import cucumber.api.java.ko.그리고;
import cucumber.api.java.ko.만약;
...
public class MultiplicationFeatureSteps {
    ...
    @만약("^사용자 ([^\\s]+)가 (\\d+)개의 ([^\\s]+) 답안을 제출한다$")
    public void 사용자가_정답을_제출한다(String userAlias, int attempts,
                              String rightOrWrong) throws Throwable {
        final int attemptSent = IntStream.range(0, attempts)
                .mapToObj(i -> app.sendAttempt(userAlias, 10, 10,
                        "정답".equals(rightOrWrong) ? 100 : 250))
                .peek(response -> lastAttemptResponse = response)
                .mapToInt(response -> response.isCorrect() ? 1 : 0)
                .sum();

        assertThat(attemptSent)
                .isEqualTo("정답".equals(rightOrWrong) ? attempts : 0)
                .withFailMessage("답안을 애플리케이션으로 전송 시 에러");
    }

    @그러면("^사용자는 답안이 ([^\\s]+)이라는 응답을 받는다$")
    public void 사용자는_응답을_받는다(String rightOrWrong) throws Throwable {
        assertThat(lastAttemptResponse.isCorrect())
                .isEqualTo("정답".equals(rightOrWrong))
                .withFailMessage("기대한 응답 " + rightOrWrong + " 답안");
    }

    @그리고("^사용자는 (\\d+)점을 얻는다$")
    public void 사용자는_점수를_얻는다(int point) throws Throwable {
        final long attemptId = lastAttemptResponse.getId();
        Thread.currentThread().sleep(2000);
        int score = app.getScoreForAttempt(attemptId).getScore();
        assertThat(score).isEqualTo(point);
    }

    @그리고("^사용자는 ([^\\s]+) 배지를 얻는다$")
    public void 사용자는_배지를_얻는다(String badgeType) throws Throwable {
        final long userId = lastAttemptResponse.getUser().getId();
        Thread.currentThread().sleep(2000);
        lastStatsResponse = app.getStatsForUser(userId);
        final List<String> userBadges = lastStatsResponse.getBadges();
    ...
    public void 사용자는_배지를_얻지_못한다() throws Throwable {
        final long userId = lastAttemptResponse.getUser().getId();
        final Stats stats = app.getStatsForUser(userId);
        final List<String> userBadges = stats.getBadges();
        if(stats.getScore() == 0){
    ...
    public void 사용자는_점수를_가지고_있다(final int points) throws Throwable {
        final long userId = lastAttemptResponse.getUser().getId();
        final int statPoints = app.getStatsForUser(userId).getScore();
